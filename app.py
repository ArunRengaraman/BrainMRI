...
# New function to validate if the image is a brain MRI
@st.cache_data
def validate_brain_mri(image):
    """
    Validate if the provided image is likely a brain MRI scan.
    
    Parameters:
    - image: PIL Image object
    
    Returns:
    - is_valid: Boolean indicating if image is likely a brain MRI
    ...
    try:
        ...
        # Feature 1: Check intensity distribution (brain MRIs tend to have specific intensity distributions)
        hist = cv2.calcHist([gray], [0], None, [256], [0, 256])
        hist_norm = hist / hist.sum()
        
        # Brain MRIs typically have a certain distribution of pixel intensities
        dark_ratio = np.sum(hist_norm[:50]) / np.sum(hist_norm)
        mid_ratio = np.sum(hist_norm[50:200]) / np.sum(hist_norm)
        
        # Feature 2: Check for edges commonly found in brain MRIs
        edges = cv2.Canny(gray, 50, 150)
        edge_density = np.count_nonzero(edges) / edges.size
        
        # Feature 3: Check structural symmetry (common in brain MRIs)
        height, width = gray.shape
        left_half = gray[:, :width//2]
        right_half = gray[:, width//2:]
        right_half_flipped = cv2.flip(right_half, 1)
        
        # Resize if halves aren't the same size
        ...
        is_brain_mri = (
            (dark_ratio > 0.1 and dark_ratio < 0.6) and  # Typical brain MRI dark area ratio
            (mid_ratio > 0.3) and                        # Sufficient mid-tone content
            (edge_density > 0.01 and edge_density < 0.1) and  # Appropriate edge density
            (symmetry_score > 0.15)                      # Some level of symmetry
        )
        
        if not is_brain_mri:
            return False, "The uploaded image does not appear to be a brain MRI scan. Please upload a valid brain MRI image."
        
        return True, ""
    
    except Exception as e:
        return False, f"Error validating image: {str(e)}. Please upload a valid brain MRI scan."
